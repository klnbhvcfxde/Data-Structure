...
// 一个问题, 真的插入数据了吗, 为什么数列节点上放的是key. 至少有key有value吧. key找到pos, 读取cells[pos]上val, 应该这样才对吧

#define MAXTABLESIZE 100000   // 允许开辟的最大散列表长度 
typedef int Index;  // index是散列表的下标
typedef int ElementType;  // 关键词类型用整型
typedef Index Position;  // 数据所在位置与散列地址是同一类型

// 散列单元状态类型，分别对应：有合法元素、空单元、有已删除元素
typedef enum {Legitimate, Empty, Deleted} EntryType;  // 定义单元状态类型
// 其实有delete函数只是没去写. 似乎delete之后info变成deleted?
// 可是find函数循环结束条件是找到(info==empty)才对吧, 后面insert调用find之后又 非非空都放上东西.
// 以info为deleted的pos出find, 那就是没找到, 不然while条件就变了
// 没找到的legitimate不放东西, deleted就放了?
...
// 创建哈希表 
HashTable CreateTable(int TableSize)  // 他给的tableSize在算完比他大的素数作为新的size之后就不用了
{
	HashTable H;
...
{
    // 这i做甚么...
    // Position Pos = i; // 看到一个版本这么写, 也不知道在做什么
...
{
	return Key % TableSize; // 除一个素数比较不容易相等
}
